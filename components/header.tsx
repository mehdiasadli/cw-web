        ...
        <div className='flex items-center justify-between h-16 lg:h-20'>
          ...
          <Link href='/' className='flex items-center space-x-2'>
            ...
            <div className='flex flex-col'>
              <span className='text-white font-bold text-lg leading-tight'>CROWN</span>
              <span className='text-crown-primary font-bold text-sm leading-tight'>WELLNESS CLUB</span>
            </div>
          </Link>
            ...
            {navLinks.map((link) => (
              <div key={link.name} className='relative group'>
                {link.type === 'page' ? (
                  ...
                  >
                    <span>{link.name}</span>
                    {link.type === 'page' && link.href !== '/' && <ExternalLink className='w-3 h-3' />}
                  </Link>
                ) : (
                  <button
                    onClick={() => handleNavClick(link.href, link.type)}
          ...
          <div className='flex items-center space-x-4'>
            ...
            <div className='relative language-dropdown'>
              ...
              {/* Language Dropdown */}
              <div
                className={`absolute top-full right-0 mt-3 min-w-[180px] py-2 bg-black/95 backdrop-blur-xl rounded-xl shadow-2xl transition-all duration-300 transform ${isLanguageDropdownOpen ? 'opacity-100 visible scale-100' : 'opacity-0 invisible scale-95'}`}
              >
            ...
            {navLinks.map((link) => (
              <div key={link.name}>
                {link.type === 'page' ? (
                  ...
                  >
                    <span>{link.name}</span>
                    {link.type === 'page' && link.href !== '/' && <ExternalLink className='w-4 h-4' />}
                  </Link>
                ) : (
                  <button
                    onClick={() => handleNavClick(link.href, link.type)}
